...
export function InvoicePaymentProgress({ quote }: Props) {
  ...
  const downPaymentDate = format(new Date(quote.poIssuedAt ?? quote.issuedAt), 'yyyy.MM.dd');
  const clientSigner =
  ...
  return (
    <div className="mx-8 rounded-xl border border-[#C9D8EF] bg-[#F5F7FC] p-5 py-6">
      ...
      <div className="mt-4 flex h-2 overflow-hidden rounded-full bg-slate-100">
        <div className="w-[30%] rounded-full bg-emerald-500" title="선금 30%" />
        <div className="w-[70%] bg-slate-200" title="잔금 70%" />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-4 text-xs">
        <div>
          <p className="font-bold text-emerald-500">✅ 선금 30% · 입금 완료</p>
        ...
        <div>
          <p className="font-bold text-[#FF9500]">⏳ 잔금 70% · 수령 확인 후</p>
